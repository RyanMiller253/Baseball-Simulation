...
# number for seeding random number generator
...
class Simulation(Char):
    def __init__(self):
        ...
        self.FSM.AddTransition("toDetermine_Runners",
                               Transition("Determine_Runners"))
...
def import_stats(self, file):
    ...
    counter = 0
    for a in self:
        a.setStats(lineup_txt[counter][2], int(lineup_txt[counter][5]), int(lineup_txt[counter][8]), int(
            lineup_txt[counter][9]), int(lineup_txt[counter][10]), int(lineup_txt[counter][11]), int(lineup_txt[counter][15]))
        # print(counter)
...
def choose_csv():
    # CHANGE THIS BACK!!!
    file_dir = os.listdir('C:/Users/rymiller/Desktop/Simulation')
    roster_arr = []
